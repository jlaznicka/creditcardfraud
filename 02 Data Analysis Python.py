...
def main(session: snp.Session): 
    ...
    df_trx_by_day = (df_cust_trx_fraud
                     .group_by(F.to_date(F.col("TX_DATETIME")))
                     .count()
                     .sort(F.col("TO_DATE(TX_DATETIME)"))
                     .select(
                         F.col("TO_DATE(TX_DATETIME)").as_("DATE"), 
                         F.col("COUNT")
                     ))
    ...
    df_cust_trx_fraud.group_by(F.col("TX_FRAUD"))\
                     .agg(F.count(F.col("TRANSACTION_ID")).as_("NB_TX_DAY"))\
                     .select(
                         F.col("TX_FRAUD"), 
                         F.col("NB_TX_DAY"), 
                         (F.call_function("RATIO_TO_REPORT", F.col("NB_TX_DAY")).over() * 100).as_("percentage")
                     )\
                     .show()
    
    # Number of fraudulent transactions per card (PLOT ON CHART)
